...
Implemented Phase 3 scope includes the provider-neutral news search Port, Fake and conditional Naver Adapters, idempotent ingestion, stored-news APIs, the conditional internal sync API, reusable HTTP Adapter contract tests, explicit `TermNewsMapping` application storage, the pure title/summary `TermNewsMatcher`, limited ID-based `TermNewsAutoMappingService` orchestration, and the conditional internal mapping rebuild API. Mapping storage is idempotent by term/article pair and only stronger evidence replaces existing evidence. Automatic mapping accepts an explicit request of at most 100 stored-news IDs, evaluates every ACTIVE term with eagerly fetched aliases in stable ID order, and aggregates CREATED/UPDATED/SKIPPED/unmatched results. News ingestion and mapping rebuild remain independently invoked. Related-news APIs, unlimited/full reprocessing, and schedulers are not implemented. Do not implement Redis popular-search behavior until Phase 4.
