...
상태: 진행 중. 뉴스 제공자 Port, 테스트용 Fake Adapter, NewsIngestionService의 멱등 저장, NewsQueryService의 목록·상세 조회까지 완료했다.
...
- 남음: 뉴스 Controller와 공개 목록·상세 API 구현
...
Controller, 내부 동기화 API를 구현하지 않는다. 다음 작은 작업은 구현된
`NewsQueryService`를 변경하지 않고 얇은 뉴스 목록·상세 Controller와 API 계약
테스트를 추가하는 것이다.
