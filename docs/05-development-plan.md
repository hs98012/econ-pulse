...
상태: 진행 중. 뉴스 제공자 Port, 테스트용 Fake Adapter, NewsIngestionService의 멱등 저장, NewsQueryService와 공개 저장 뉴스 목록·상세 API까지 완료했다.
...
현재 실제 네이버 뉴스 API, 외부 HTTP 호출, 자동 매핑과 관련 뉴스 API, 스케줄러,
내부 동기화 API는 구현하지 않았다. 다음 작은 작업은 provider 중립 Port를 유지한
채 실제 외부 News Adapter의 요청·응답 매핑 계약을 설계하는 것이다.
