...
- DB에서 입력을 조회하고 후보를 `TermNewsMappingService` 저장으로 연결하는 자동 매핑 Application 흐름, 관련 뉴스 조회와 재처리 기능은 아직 구현하지 않았다.
...
- `GET /api/v1/terms/{termId}/news`는 ACTIVE 용어의 관련 뉴스를 `publishedAt DESC, newsArticle.id DESC`로 DB 페이징한다. 응답은 뉴스 공개 필드, match type과 confidence score만 포함하며 매핑이 없으면 빈 페이지다.
- 뉴스 수집 후 자동 호출, 뉴스별 연결 용어 조회, ID 없는 전체 재처리와 스케줄러는 아직 구현하지 않았다.
