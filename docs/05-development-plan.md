...
상태: 진행 중. 뉴스 Provider, 멱등 수집·조회 API, 내부 동기화 API, 명시적 TermNewsMapping 저장, 순수 후보 계산과 제한된 뉴스 ID 기반 자동 매핑까지 완료했다.
...
- 남음: 뉴스 수집 후 자동 매핑 연결 여부 결정과 내부 전체 재처리 API 구현
...
자동 매핑 연결은 구현됐지만 자동 테스트는 실제 외부 인터넷이나 자격 증명을 사용하지
않는다. 뉴스 수집 후 자동 호출, 관련 뉴스 API, 내부 전체 재처리와 스케줄러는 구현하지
