...
상태: 진행 중. 뉴스 Provider, 멱등 수집·조회 API, 내부 동기화 API, 명시적 TermNewsMapping 저장, 순수 후보 계산, 제한된 뉴스 ID 기반 자동 매핑과 조건부 내부 재처리 API까지 완료했다.
...
- 남음: 뉴스별 연결 용어 조회
- 남음: 뉴스 수집 후 자동 매핑 연결 여부 결정과 ID 없는 전체 재처리 Job 설계
- 남음: 수집 스케줄러와 실제 Provider 기반 운영 동기화 흐름 구현

### 성공 기준

- 일부 충족: 같은 뉴스와 매핑 명령을 반복해도 중복 행이 생기지 않는다. 동시 신규 매핑 충돌은 명시적 예외로 실패하며 자동 재시도는 아직 없다.
- 충족: 저장 뉴스와 용어별 관련 뉴스는 발행일과 ID 내림차순으로 DB 페이징 조회된다.
- 일부 충족: 외부 제공자 Port와 Fake/Naver Adapter의 검색, 정렬, 페이징, 예외 모델과 수집 저장 테스트가 통과한다. 실제 자격 증명을 사용한 운영 smoke 검증은 별도다.

현재 Naver HTTP Adapter, 명시적 매핑 저장, 순수 후보 계산과 최대 100개 뉴스의 DB 기반
자동 매핑 연결과 지정 ID 내부 재처리 API는 구현됐지만 자동 테스트는 실제 외부 인터넷이나
자격 증명을 사용하지 않는다. 용어별 관련 뉴스 공개 조회는 완료했지만 뉴스 수집 후 자동
호출, 뉴스별 연결 용어 API, ID 없는 전체 재처리와 스케줄러는 구현하지 않았다. 현재
Phase 3 계획에 수집 스케줄러가 명시적으로 남아 있으므로 Phase 3는 아직 진행 중이다.
다음 작은 작업은 기본 비활성 수집 스케줄러의 실행 경계와 중복 실행 정책을 문서로 먼저
고정하는 것이다. Phase 3 완료 후 다음 구현 단계는 Phase 4 Redis 인기 검색어다.
