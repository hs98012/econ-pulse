...
Implemented Phase 3 scope includes the provider-neutral news search Port, Fake and conditional Naver Adapters, idempotent ingestion, stored-news APIs, the conditional internal sync API, reusable HTTP Adapter contract tests, explicit `TermNewsMapping` application storage, the pure title/summary `TermNewsMatcher`, and limited ID-based `TermNewsAutoMappingService` orchestration. Mapping storage is idempotent by term/article pair and only stronger evidence replaces existing evidence. Automatic mapping accepts at most 100 unique stored-news IDs, evaluates every ACTIVE term with eagerly fetched aliases in stable ID order, and aggregates CREATED/UPDATED/SKIPPED/unmatched results. News ingestion does not invoke mapping automatically. Related-news APIs, full reprocessing APIs, and schedulers are not implemented. Do not implement Redis popular-search behavior until Phase 4.
