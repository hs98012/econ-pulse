...
- 남음: Phase 4 핵심 E2E 정리와 완료 여부 검토
- 남음: `PopularTermSnapshot` 주기 저장, 과거 조회와 스케줄러

### 성공 기준

- 충족: 현재 독립 저장 경계에서 동시 검색 증가가 유실되지 않는다.
...
- 미착수: 동일 시점 Snapshot 중복 방지는 Snapshot 저장 단계에서 구현한다.
