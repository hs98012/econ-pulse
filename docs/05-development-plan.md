...
상태: 진행 중. 뉴스 제공자 Port, 테스트용 Fake Adapter, NewsIngestionService의 멱등 저장, NewsQueryService, 공개 저장 뉴스 조회 API와 조건부 내부 동기화 API까지 완료했다.
...
현재 실제 네이버 뉴스 API, 운영 HTTP Adapter, 외부 인터넷 호출, 자동 매핑과 관련
뉴스 API, 스케줄러는 구현하지 않았다. 다음 작은 작업은 확정된 공통 계약을 상속하는
Naver Adapter의 provider별 DTO와 요청·응답 매핑을 구현하는 것이다.
