...
상태: 진행 중. 뉴스 Provider, 멱등 수집·조회 API, 내부 동기화 API와 명시적 TermNewsMapping 저장 경계까지 완료했다.
...
현재 Naver HTTP Adapter와 명시적 매핑 저장은 구현됐지만 자동 테스트는 실제 외부
인터넷이나 자격 증명을 사용하지 않는다. 자동 매칭, 관련 뉴스 API, 내부 재처리와
스케줄러는 구현하지 않았다. 다음 작은 작업은 제목·요약 문자열을 입력받아 일치 후보를
계산하는 순수 자동 매칭 도메인 정책이다.
