...
- 뉴스 제공자 포트와 테스트용 어댑터 구현
- `NewsArticle`, `TermNewsMapping` 저장 및 조회 구현
- 이름·별칭 기반 매핑과 내부 재처리 API 구현

### 성공 기준

- 같은 뉴스와 매핑을 반복 처리해도 중복 행이 생기지 않는다.
- 관련 뉴스가 발행일 최신순으로 조회된다.
- 외부 제공자 실패와 부분 데이터 테스트가 통과한다.
