...
상태: 진행 중. 뉴스 Provider, 멱등 수집·조회 API, 내부 동기화 API, 명시적 TermNewsMapping 저장 경계와 한 쌍의 순수 매칭 후보 계산까지 완료했다.
...
- 남음: DB 조회·저장과 순수 후보 계산을 연결하는 자동 매핑 Application 흐름과 내부 재처리 API 구현
...
현재 Naver HTTP Adapter, 명시적 매핑 저장과 순수 매칭 후보 계산은 구현됐지만 자동
테스트는 실제 외부 인터넷이나 자격 증명을 사용하지 않는다. DB 기반 자동 매핑 연결,
관련 뉴스 API, 내부 재처리와 스케줄러는 구현하지 않았다. 다음 작은 작업은 명시적인
단건 입력 경계에서 엔티티를 입력 모델로 변환하고 후보를 기존 저장 명령으로 연결하는
Application Service다. 전체 재처리나 Controller 없이 단건 흐름부터 구현한다.
