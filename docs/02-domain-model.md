...
현재 영속 Aggregate와 Entity는 `domain` 패키지에서 Jakarta Persistence annotation을 직접
사용하는 실용적 JPA 모델이다. 따라서 전체 domain이 JPA와 완전히 분리된 구조는 아니다.
대신 순수 `TermNewsMatcher` 입력·정책, Application Port와 관측 Port는 Spring Web, Redis,
Servlet과 Micrometer 구현 타입에서 분리하며 ArchUnit으로 이 경계를 검증한다. JPA 모델을
별도 persistence entity로 분리하는 대규모 구조 변경은 1.0.0 완료 범위가 아니다.
...
- `EconomicTermService`: 용어 생성·수정·비활성화, 목록·검색과 ACTIVE 상세 결과 조회
- `EconomicTermDetailFacade`: 공개 상세 결과가 생성된 뒤 `PopularTermService`로 조회 성공을
  기록한다. 예상한 Redis unavailable만 fail-open하며 API DTO나 Redis Adapter를 사용하지 않는다.
...
- `PopularTermQueryService`: UTC 오늘 또는 명시 날짜의 Redis 순위를 ACTIVE MySQL 용어와
  결합하고 누락·비활성 항목을 제외한 뒤 공개 rank를 다시 부여한다.
- `PopularTermController`: 오늘의 인기 용어 배열을 공개하며 Query Service에만 의존한다.
- `PopularTermSnapshotService`: 향후 Redis 순위를 MySQL에 주기적으로 저장할 운영 개선 backlog
...
- `GET /api/v1/terms/popular`는 Controller에서 날짜를 계산하지 않고 Query Service의 UTC
  Clock을 사용한다. Redis 장애는 공통 오류 처리에서 503으로 변환하며 Snapshot fallback은 없다.
- 공개 `GET /api/v1/terms/{termId}` 상세 조회 성공만 요청마다 score를 1 증가시킨다.
  목록·검색·관련 뉴스·인기 순위 조회와 내부 API는 기록하지 않는다.
- 상세 조회 실패는 기록 전에 전파한다. 조회 성공 후 Redis 기록 unavailable은 warning만
  남기고 상세 결과를 유지하며, 다른 저장소 손상이나 예상하지 못한 오류는 숨기지 않는다.
- 사용자·세션·IP별 중복 제거는 없고 MySQL 읽기와 Redis 기록을 분산 트랜잭션으로 묶지 않는다.
- 이 실시간 흐름과 공개 API까지가 완료된 Phase 4 범위다. Snapshot 저장·과거 조회와
  스케줄러는 완료 범위 밖이며 `docs/15-backlog.md`에서 관리한다.
