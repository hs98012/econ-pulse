...
상태: 진행 중. 뉴스 제공자 Port, Fake/Naver Adapter, 멱등 수집 저장, 저장 뉴스 조회 API와 조건부 내부 동기화 API까지 완료했다.
...
현재 Naver HTTP Adapter는 구현됐지만 자동 테스트는 실제 외부 인터넷이나 자격 증명을
사용하지 않는다. 자동 매핑과 관련 뉴스 API, 스케줄러는 구현하지 않았다. 다음 작은
작업은 `TermNewsMapping`의 멱등 저장 application 기능이다.
