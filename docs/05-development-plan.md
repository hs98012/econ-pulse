...
상태: 진행 중. 첫 작업인 뉴스 제공자 Port와 테스트용 Fake Adapter만 완료했다.
...
- 남음: `NewsArticle`, `TermNewsMapping` 저장 및 조회 구현
- 남음: 이름·별칭 기반 매핑과 내부 재처리 API 구현
- 남음: 수집 스케줄러와 운영용 동기화 흐름 구현

### 성공 기준

- 진행 중: 같은 뉴스와 매핑을 반복 처리해도 중복 행이 생기지 않는다.
- 진행 중: 관련 뉴스가 발행일 최신순으로 조회된다.
- 일부 충족: 외부 제공자 Port/Fake Adapter의 검색, 정렬, 페이징, 예외 모델 테스트가 통과한다.

이번 단계에서는 실제 네이버 뉴스 API, 외부 HTTP 호출, 뉴스 저장, 자동 매핑,
스케줄러, 내부 동기화 API를 구현하지 않는다. 다음 작은 작업은 실제 외부 Adapter
없이 Fake Provider를 사용하는 뉴스 수집 application service skeleton을 정의하는 것이다.
