...
- 일부 충족: 저장 뉴스는 발행일과 ID 내림차순으로 조회된다. 용어별 관련 뉴스 조회는 아직 남아 있다.
...
자격 증명을 사용하지 않는다. 뉴스 수집 후 자동 호출, 관련 뉴스 API, ID 없는 전체 재처리와 스케줄러는 구현하지
않았다. 다음 작은 작업은 관련 뉴스 공개 API보다 먼저 Repository의 용어별 매핑 조회를
Application DTO로 반환하는 읽기 전용 단위 기능이다.
