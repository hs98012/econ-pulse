...
- 모든 HTTP 응답은 `X-Request-Id` 헤더를 포함한다. `[A-Za-z0-9._-]{8,128}` 형식의
  클라이언트 값은 재사용하고, 없거나 잘못된 값은 서버 UUID로 대체한다.
- 성공·오류 응답 모두 같은 요청 ID를 사용하며 오류 JSON 구조에는 requestId 필드를
  추가하지 않는다. 운영 문의와 장애 보고에는 응답의 `X-Request-Id` 제공을 권장한다.
